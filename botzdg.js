  ...
  if (media && (mimetype === 'application/pdf' || mimetype === 'application/msword' || mimetype === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')) {
    client.sendMessage(numberZDG, media, { caption: caption, sendMediaAsDocument: true })
      .then(response => {
        res.status(200).json({
          status: true,
          message: 'BOT-ZDG Documento enviado',
          response: response
        });
      }).catch(err => {
        res.status(500).json({
          status: false,
          message: 'BOT-ZDG Documento não enviado',
          response: err.text
        });
      });
  } else if (media) {
    // This is an image or other non-document media, so send it as a media message
    ...
    // No media, so send a text message
    client.sendMessage(numberZDG, caption)
      .then(response => {
        res.status(200).json({
          status: true,
          message: 'BOT-ZDG Mensagem enviada',
